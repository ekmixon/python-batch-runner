...
_key_err_msg = "Config object does not store key: {}"
...
class Config:
    ...
    def __init__(self):
        self._attr = {
            ...
            'app_start_time'       : { 'type': str , 'preserve': True,  'env': None                       , 'value': None, 'default': datetime.now().strftime("%Y-%m-%d %H:%M:%S") },
    ...
    def __getitem__(self, key):
        ...
        if detl["env"] and os.environ.get(detl["env"]) is not None:
            if (
                attr_type == bool
                and os.environ.get(detl["env"], "TRUE").upper() == "FALSE"
            ):
                return False
    ...
    def source_config_file(self, config_file):
        ...
        if not str_path or not os.path.isfile(str_path):
            raise FileNotFoundError(
                "Configuration file {} does not exist.".format(str_path)
            )
